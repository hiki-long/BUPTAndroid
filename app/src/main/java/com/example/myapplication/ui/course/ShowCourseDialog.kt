...
class ShowCourseDialog : DialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View?{
        ...
        if (arguments != null) {
            ...
            courseday.text = requireArguments().getString("week")
            val courssbegin = view.findViewById<TextView>(R.id.edit_course_begin)
            courssbegin.text = requireArguments().getInt("start").toString()
            val coursend = view.findViewById<TextView>(R.id.edit_course_end)
            coursend.text = requireArguments().getInt("end").toString()
